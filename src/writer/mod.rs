...
impl<E: PropertyAccess> Writer<E> {
    ...
    /// Writes a property line from the header:
...
/// # Binary
impl<E: PropertyAccess> Writer<E> {
    fn write_binary_list_len<T: Write, B: ByteOrder>(
        &self,
        out: &mut T,
        ...
        match *index_type {
            ScalarType::Char => match i8::try_from(vec_len) {
                ...
                Err(_) => Err(io::Error::new(
                    ErrorKind::InvalidInput,
                    format!(
                        "List property '{}' has {} entries, which exceeds the range of '{}' list indices.",
                        property_name, vec_len, "char"
                    ),
                )),
            },
            ...
            ScalarType::UInt => match u32::try_from(vec_len) {
                ...
                Err(_) => Err(io::Error::new(
                    ...
                    format!(
                        "List property '{}' has {} entries, which exceeds the range of '{}' list indices.",
                        property_name, vec_len, "uint"
                    ),
                )),
            },
            ScalarType::Float => Err(io::Error::new(
                ErrorKind::InvalidInput,
                "Index of list must be an integer type, float declared in PropertyType.",
            )),
            ScalarType::Double => Err(io::Error::new(
                ErrorKind::InvalidInput,
                "Index of list must be an integer type, double declared in PropertyType.",
            )),
        }
    }
